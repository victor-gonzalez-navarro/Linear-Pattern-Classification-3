...
for k0=1:K_folds
    ...
    d = 1;
    for p=[1,10,20,100]
        knn_out = predict_parzen(X_train_red,Labels_train_red, N_classes, p, X_valid);
        knn_Pe_train=sum(Labels_valid ~= knn_out)/length(Labels_valid);
        KNN_Errors_Train(k0,d) = knn_Pe_train;
        
        
        knn_out2 = predict_parzen(X_train_red,Labels_train_red, N_classes, p, X_train_red);
        knn_Pe_train2=sum(Labels_train_red ~= knn_out2)/length(Labels_train_red);
        KNN_Errors_Train2(k0,d) = knn_Pe_train2;
        d= d +1;
    end
end

figure;
KNN_Errors_Sum = sum(KNN_Errors_Train,1)/2;
plot(KNN_Errors_Sum);

figure;
KNN_Errors_Sum_2 = sum(KNN_Errors_Train2,1)/2;
plot(KNN_Errors_Sum_2);

%% -----------------------------------------------------------------
knnclasstest = fitcknn(X_test,Labels_test,'NumNeighbors',20);
